...
def Load():
    ...
    if not os.path.exists(filename):
        print(Fore.LIGHTRED_EX + "combos.txt not found in the current directory.")
        time.sleep(2)
        return
    ...
    try:
        with open(filename, 'r', encoding='utf-8', errors='ignore') as e:
            lines = [ln.strip() for ln in e.readlines()]
            ...
            for ln in lines:
                ...
                if ln not in seen:
                    ...
                    cleaned.append(ln)
            Combos = cleaned
            dupes_removed = len(lines) - len(cleaned)
            print(Fore.LIGHTBLUE_EX + f"[{dupes_removed}] Dupes/empty lines Removed.")
            print(Fore.LIGHTBLUE_EX + f"[{len(Combos)}] Combos Loaded from combos.txt.")
    except Exception as ex:
        print(Fore.LIGHTRED_EX + f"Error loading combos.txt: {ex}")
        time.sleep(2)
        return
...
def Proxys():
    ...
    if not os.path.exists(filename):
        print(Fore.LIGHTRED_EX + "proxies.txt not found in the current directory.")
        time.sleep(1)
        return
    ...
    try:
        with open(filename, 'r', encoding='utf-8', errors='ignore') as e:
            ...
            for ln in lines:
                if not ln:
                    continue
                proxyline = ln.split()[0].replace('\n', '')
                if proxyline not in seen:
                    seen.add(proxyline)
                    cleaned.append(proxyline)
            proxylist = cleaned[:]
            print(Fore.LIGHTBLUE_EX + f"Loaded [{len(proxylist)}] proxies from proxies.txt.")
            time.sleep(1)
    except Exception as ex:
        print(Fore.LIGHTRED_EX + "Error loading proxies.txt: " + str(ex))
        time.sleep(2)
        return
